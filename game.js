...
const gameBoard = (function (){
    ...
    const checkBoard = () => {
        ...
        for (let i = 0; i < 3; i++) {
            let rowIndex = i*3;
            check = check || (board[i] === board[i+3] && board[i] === board[i+6] && board[i] != undefined)  ? true : false;     // check column
            check = check || (board[rowIndex] === board[rowIndex+1] && board[rowIndex] === board[rowIndex+2] && board[rowIndex] != undefined) ? true : false; // check row              
        }
        return check;
    }
...
function somebodyPlay(e){
    gameBoard.nextMove() ? playerOne.play(e.currentTarget.position) : playerTwo.play(e.currentTarget.position);
    console.log('---------------------------------------------');
